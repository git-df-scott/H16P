# Second strike: exact original-integral reconstruction and a sign exclusion

2026-09-04. Owned derivation and bounded replay: `q4_reconstruction.py`.
The inherited original basis is
`(hI00,I10,I01,2I−1,0+3κhI−1,1)`, with positive area orientation.
The inherited bounds exclude five zeros in the `β1=0` chart. Every possible
five-zero direction is therefore normalized to `β1=1` throughout these
notes, with no change to its roots.
...
The **minus sign** is required. The plus sign printed in Zhao equation (24)
...
The root agent derived the following further reduction; its signs and
endpoint limits have been checked against (R1)–(R7). It isolates the next
construction problem without claiming its inequalities are satisfiable.

Let `0<h1<h2<h3<1` be the three simple zeros of `H`. Define
...
\[
\boxed{\max\{0,V(h_2)\}<P_0<\min\{V(h_1),V(h_3)\}}. \tag{S1}
\]
...
The exact certificate in `notes_certificate_second.md` places all of this
...
### Frozen numerical shooting follow-up

The root agent subsequently froze the rational shooting parameter
`a=189417314263391/400000000000000`, with
`k=400000000000000/210582685736609`. The numerical record
`data/second_green_shoot.json` reports `P0≈2.1646953843e−8` and four `P`
crossings near `0.14446487,0.38212709,0.58385951,0.81006267`. The four
corresponding `Z` extrema are all about `−0.00457918`, so (S2) fails.
This observation is numerical only; the all-kappa coefficient-box theorem
above independently certifies failure of five original zeros. The final
root document includes the complete frozen table. No new exploration was
performed to integrate this result.
