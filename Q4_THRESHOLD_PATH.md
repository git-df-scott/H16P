# Q4 threshold path: exact crossing and a certified late-root box

2026-09-04. **PROVED:** an analytic primitive-anchor path crosses the
necessary threshold exactly. **RIGOROUS COMPUTATION:** an explicit rational
point and coefficient box have their first primitive root beyond `23/32`.
**PROVED:** the same path has explicit, highly ill-conditioned endpoint
asymptotics. These results do not supply five original Abelian-integral zeros
or a positive Green maximum. The next reconstruction gate is analyzed in
[Q4_GREEN_MAX_3.md](Q4_GREEN_MAX_3.md).
...
The other inherited necessary condition is also strict:

\[
 \boxed{\kappa>21636/19043}.
\]

The second-strike exclusion covers the entire closed-ended range
\(1<\kappa\le21636/19043\). The dimensionless reconstruction parameter
\(a=1-1/\kappa\) must therefore satisfy \(a>2593/21636\).
These are necessary conditions for five distinct original zeros, which are
simple by the multiplicity-five theorem. Neither threshold certifies any
original zero. Their exact regression identities are checked in
[q4/test_q4_third.py](q4/test_q4_third.py), and their proofs are retained in
...
proved in [Q4_LOBE_REGION.md](Q4_LOBE_REGION.md). Define
...
The implementation in [q4/q4_threshold_path.py](q4/q4_threshold_path.py) exposes
...
[q4/data/third_threshold_certificate.json](q4/data/third_threshold_certificate.json).
...
or establish that the remaining two-zero allowance is attained. The original PF/Green analysis of this path and its permitted center
parameters is recorded in [Q4_GREEN_MAX_3.md](Q4_GREEN_MAX_3.md). Any original
five-sign lead must still be independently certified in the original
normalization.
