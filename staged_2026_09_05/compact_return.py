...
def evaluate(req):
    ...
    if remote:
        eq=np.roots([c-12.2,alpha-22.2-beta,2*alpha-10-beta,alpha])
        real=[a.real for a in eq if abs(a.imag)<1e-8 and a.real<-1]
        if len(real)!=1 or r>=real[0]:raise ValueError('remote point is not left of its equilibrium')
    if -10*r*r+2.2*r*y0+c*y0*y0+alpha*r+beta*y0>=0:
        raise ValueError('initial flux has incorrect orientation')
    ...
    if -10*R*R+2.2*R*yR+c*yR*yR+alpha*R+beta*yR>=0:
        raise ValueError('final flux has incorrect orientation')
    ...
    return dict(status='NUMERICAL_ONLY',r=r,c=c,alpha=alpha,beta=beta,
                ...
                multiplier=math.exp(L),divergence_exponential=math.exp(L),
                multiplier_interpretation='Floquet multiplier only at a fixed point',
                period=float(T),rescaled_period=now,
