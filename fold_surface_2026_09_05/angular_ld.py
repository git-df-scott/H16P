...
import mpmath as mp
HERE=Path(__file__).resolve().parent;source=HERE/'angular_ld.cpp'
exe=Path('/tmp')/('h16_angular_'+hashlib.sha256(source.read_bytes()).hexdigest()[:16])
if not exe.exists():subprocess.run(['g++','-O2','-std=c++17',str(source),'-o',str(exe)],check=True,capture_output=True)
mp.mp.dps=35
def num(x):
...
data=' '.join(mp.nstr(num(q[k]),33) for k in ('r','c','K'))+' '+str(q.get('tol','2e-17'))+'\n'
