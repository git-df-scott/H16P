...
import mpmath as mp
HERE=Path(__file__).resolve().parent;source=HERE/'angular_quad.cpp'
exe=Path('/tmp')/('h16_angular_quad_'+hashlib.sha256(source.read_bytes()).hexdigest()[:16])
...
data=' '.join(mp.nstr(num(q[k]),45) for k in ('r','c','K'))+' '+str(q.get('tol','2e-17'))+'\n'
