...
int main(){
 R r,tol;std::string sr,sc,sk,st;std::cin>>sr>>sc>>sk>>st;r=strtoflt128(sr.c_str(),0);c=strtoflt128(sc.c_str(),0);K=strtoflt128(sk.c_str(),0);tol=strtoflt128(st.c_str(),0);R start=std::clock()/(R)CLOCKS_PER_SEC;
 try{
 al=-K;ac=0;ak=-1;
 ...
 emit("r",r);emit("c",c);emit("m",K);emit("K",K*(11*c-5)/5-42);emit("alpha",al);emit("return_coordinate",RR);
