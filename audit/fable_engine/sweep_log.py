...
def make(u):
    if fam == 'mvneutral':
        da = (2*u[0]-1)*0.05; b = 0.2+1.6*u[1]; a = -1+da
...
def count_nest_log(c, pt, other, umax=UMAX):
    ...
    u1, S, st = rm.returns_log(c[None], np.array([pt]), u0[None], th0=th, rtol=1e-12, umax=umax+5, Smax=1e6, maxsteps=3_000_000)
    ok = st[0] == 0; k = len(u0) if ok.all() else int(np.argmin(ok))
    D = (u1[0, :k]-u0[:k]); uu = u0[:k]
    if 1 <= k < len(u0):   # edge bisection in u
        lo, hi = u0[k-1], u0[k]
        for _ in range(8):
            mid = 0.5*(lo+hi); v1, _, s1 = rm.returns_log(c[None], np.array([pt]), np.array([[mid]]), th0=th, rtol=1e-12, umax=umax+5, Smax=1e6, maxsteps=3_000_000)
            if s1[0, 0] == 0: lo = mid; De = v1[0, 0]-mid
            else: hi = mid
        if lo > u0[k-1]: uu = np.append(uu, lo); D = np.append(D, De)
