// Binary128 independent angular shooting with modified-midpoint extrapolation.
...
R c,K,al,ac,ak,beta,theta0,span,qxy=2.2Q;int nfev=0;
A rhs(R s,const A&z){
 ++nfev;R w=z[0],M=z[1],C=cosq(theta0-s),S=sinq(theta0-s),E=expq(w);
 R h1=(1+al)*C*S+beta*S*S,g1=al*C*C-S*S+beta*C*S,p2=C*C+C*S,q2=-10*C*C+qxy*C*S+c*S*S;
...
int main(){
 R r,tol;std::string sr,sc,sk,sb,st,sth,sspan;std::cin>>sr>>sc>>sk>>sb>>st>>sth>>sspan;theta0=strtoflt128(sth.c_str(),0);span=strtoflt128(sspan.c_str(),0);beta=strtoflt128(sb.c_str(),0);r=strtoflt128(sr.c_str(),0);c=strtoflt128(sc.c_str(),0);K=strtoflt128(sk.c_str(),0);tol=strtoflt128(st.c_str(),0);std::string sq;std::cin>>sq;qxy=strtoflt128(sq.c_str(),0);R start=std::clock()/(R)CLOCKS_PER_SEC;
 try{
 al=-K;ac=0;ak=-1;
 if(r<=0||al>=0)throw std::runtime_error("focus/section gate");
 R s=0,end=(span>0?span:2*acosq(-1.L)),H=.01Q;A y{logq(r),0,0,0,0,0,0,0,0};int steps=0,reject=0;
 while(s<end){
 ...
 R RR=expq(y[0]),aa=expq(y[1]);
 auto cross=[&](R rad,R th){R C=cosq(th),S=sinq(th);R g1=al*C*C-S*S+beta*C*S;R p2=C*C+C*S,q2=-10*C*C+qxy*C*S+c*S*S;return rad*(g1+rad*(C*q2-S*p2));};
 R Q0=cross(r,theta0),Qf=cross(RR,theta0-end);
 R fluxA=r/RR*Q0/Qf*expq(y[8]);
 std::cout<<std::setprecision(23)<<"{\"status\":\"NUMERICAL_ONLY\",\"section\":\"arbitrary ray; clockwise; span=0 means full turn\"";
 auto emit=[&](const char*k,R v){char buf[100];quadmath_snprintf(buf,sizeof(buf),"%.36Qg",v);std::cout<<",\""<<k<<"\":\""<<buf<<"\"";};
 emit("qxy",qxy);emit("theta0",theta0);emit("span",end);emit("beta",beta);emit("r",r);emit("c",c);emit("m",K);emit("K",K*(11*c-5)/5-42);emit("alpha",al);emit("return_coordinate",RR);
