...
R c,K,al,ac,ak,beta,theta0,span;int nfev=0;
A rhs(R s,const A&z){
...
int main(){
 ...
 auto cross=[&](R rad,R th){R C=cosq(th),S=sinq(th);R g1=al*C*C-S*S+beta*C*S;R p2=C*C+C*S,q2=-10*C*C+2.2Q*C*S+c*S*S;return rad*(g1+rad*(C*q2-S*p2));};
 ...
 emit("theta0",theta0);emit("span",end);emit("beta",beta);emit("r",r);emit("c",c);emit("m",K);emit("K",K*(11*c-5)/5-42);emit("alpha",al);emit("return_coordinate",RR);
