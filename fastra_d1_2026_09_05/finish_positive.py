...
shoot=[json.loads(l)['result'] for l in (HERE/'shooting.jsonl').read_text().splitlines()]
a=next(a for a in reversed(shoot) if a.get('status')=='NUMERICAL_TWO_HALF_PASSAGES' and abs(float(mp.log(mp.mpf(a['r'])))-40)<1e-10)
for index,z in [(24,40.),(25,42.)]:
 if z==42:
  b,h=correct(mp.mpf(z),mp.mpf(a['c'])+mp.mpf('.00035'),mp.mpf(a['K'])+mp.mpf('.007'),'K','z',tol='2e-27')
  if b is None:
   append('events_positive_infinity.jsonl',dict(status='CORRECTOR_UNRESOLVED',target_log_r=z,history=h));break
  a=b
 ...
 # Use actual-m derivative from the archived K-chart: F_c|m = F_c|K + F_K * (11m/5).
 Fc=mp.mpf(a['F_c'])+mp.mpf(a['F_K'])*11*M/5
