...
s=json.loads((HERE/'summary.json').read_text());v=json.loads((HERE/'verified_precursor.json').read_text())
lines=[]
for name,title in [('positive_center','Positive K toward center'),('positive_infinity','Positive K toward infinity'),('negative','Negative K, continued in m')]:
 e=s['events'][name];a=e['endpoint'];lines.append(f"| {title} | {e['accepted']} | {float(a['c']):.15g} | {float(a['K']):.15g} | {float(a['r']):.12g} |")
roots='\n'.join(f"| {a['where']} | {a['r']:.12g} | {a['stability']} | {a['multiplier']:.12g} |" for a in v['roots'])
report=f'''# FASTRA D1 — fold sheets with full compactified counts
...
Negative endpoint actual coefficient: m = {s['events']['negative']['endpoint'].get('m')};
...
Maximum on the tested **beta=0 pair-present fields: {s['max_beta_zero_pair_count']}**.
Maximum after the tested Hopf steps: **{s['max_hopf_count']}**.
...
outside/terminal comparisons have {s['sign_comparison'].get('agree',0)} agreements,
{s['sign_comparison'].get('disagree',0)} disagreements, and
{s['sign_comparison'].get('unresolved',0)} unresolved comparisons. A terminal
...
There are **{sum(e['accepted'] for e in s['events'].values())} accepted fold points**,
**{s['raw_field_records']} appended full-counter field records**, and
**{s['unique_fields']} distinct named fields** including controls and precision
...
'''
(ROOT/'FASTRA_D1_REPORT_2026_09_05.md').write_text(report)
