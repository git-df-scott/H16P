...
R c,K,al,ac,ak,beta;int nfev=0;R direction=1;
A rhs(R s,const A&z){
...
int main(){R r,tol;std::string sr,sc,sk,sb,st;std::cin>>sr>>sc>>sk>>sb>>st;beta=strtoflt128(sb.c_str(),0);r=strtoflt128(sr.c_str(),0);c=strtoflt128(sc.c_str(),0);K=strtoflt128(sk.c_str(),0);tol=strtoflt128(st.c_str(),0);R start=std::clock()/(R)CLOCKS_PER_SEC;
try{
 al=-K;ac=0;ak=-1;
