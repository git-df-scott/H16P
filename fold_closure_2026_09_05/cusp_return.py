...
import mpmath as mp
HERE=Path(__file__).resolve().parent;source=HERE/'cusp_return.cpp'
exe=Path('/tmp')/('h16_cusp_'+hashlib.sha256(source.read_bytes()).hexdigest()[:16])
...
data=' '.join(mp.nstr(num(q[k]),33) for k in ('r','c','K','B'))+' '+str(q.get('tol','2e-17'))+'\n'
