...
def half(a,b,e0,e1,e2,side,T,sign,rtol=3e-12):
    ...
    def rhs(t,s):
        ...
        dv=((a+2)*u*u+b+side*e2*u+(side*(1-b)+e1*u)*iy+C*iy*iy-q*u)/(ch*ch+iy*iy)
        dz=(q-2*u)*ch/(ch*ch+iy*iy)
...
if __name__=='__main__':
    ...
    out['counts']=COUNTS.copy();(P/'reversible_log_return_v2_check.json').write_text(json.dumps(out,indent=2)+'\n')
